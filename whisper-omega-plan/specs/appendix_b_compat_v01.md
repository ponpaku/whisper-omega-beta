...
### 2.1 実装上の扱い基準
- `同等`: warning なしで受理する
- `部分互換`: mapping 先が明確なら受理し、必要に応じて warning を出す
- `部分互換(no-op)`: 互換上は受理するが、現時点で効果がない場合は warning を出す
- `非互換`: silent ignore せず usage error または machine-readable failure に寄せる
...
| `--diarize` | `--require-diarization` + `pyannote` backend 設定 | 部分互換 | WhisperX 互換を優先し、compat frontend では pyannote を既定化 | `channel` / `nemo` は `omega transcribe` で明示指定する | CLI-006 |
| `--highlight_words` | 互換受理のみ | 部分互換 | warning 付き no-op 互換 | 将来 subtitle emphasis と接続予定 | CLI-007 |
...
## 7. 未確定事項
1. WhisperX 側の `--output_format` 命名を互換 alias としてどこまで維持するか
2. `metadata` 必須化を「非互換」ではなく「拡張互換」と表現するか
